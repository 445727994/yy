...
class Edit {
	...
	protected function return_input($data) {
		...
		$html = '<input type="input" name="' . $data['field_name'] . '" class="layui-input" ' . $validate . '  value="{$info.' . $data["field_name"] . '}">';
	...
	protected function return_textarea($data) {
		...
		if (isset($data['yhc_editor'])) {
			$html = '{:get_editor("' . $data['yhc_editor'] . '",$info.' . $data["field_name"] . ')}';
		} else {
			$html = '<textarea name="' . $data['field_name'] . '" class="layui-textarea"   ' . $validate . ' >{$info.' . $data["field_name"] . '}</textarea>';
		}
		return $this->html_head($data['yhc_name']) . $html . $this->html_foot();
	...
	protected function return_switch($data) {
		...
		$lay_check = '{if($info["' . $data["field_name"] . '"] eq 1)}checked="checked"{/if}';
		$html = '<input type="checkbox" lay-skin="switch" lay-text="' . $lay_text . '" name="' . $data['field_name'] . '" ' . $lay_check . '   value="{$info.' . $data["field_name"] . '}">';
		return $this->html_head($data['yhc_name']) . $html . $this->html_foot();
	}
	protected function return_redio($data) {
		if (!isset($data['yhc_validate'])) {
			...
			foreach ($data['yhc_value'] as $key => $value) {
				$html .= '<input type="radio"  name="' . $data['field_name'] . '"  lay-skin="switch"  value="{$info.' . $data["field_name"] . '}"  title="' . $value . '">';
			}
			return $this->html_head($data['yhc_name']) . $html . $this->html_foot();
		}
		if (in_array($data['yhc_validate'], $this->validate_arr));
		$html = '<input type="checkbox" lay-skin="switch" name="' . $data['field_name'] . '"   value="{$info.' . $data["field_name"] . '}">';
		return $this->html_head($data['yhc_name']) . $html . $this->html_foot();
	}
	protected function return_uploadimg($data) {
		...
		$html .= '<img class="layui-upload-img" id="img' . $data['field_name'] . '"  src="{$info.' . $data['field_name'] . '|get_img}" style="max-width: 200px;">' . "\n";
		$html .= '<p></p>' . "\n";
		$html .= '</div>' . "\n";
		$html .= '<input type="hidden" name="' . $data['field_name'] . '" value="{$info.' . $data['field_name'] . '}" id="' . $data['field_name'] . '">' . "\n";
		$html .= "<script>" . "\n";
	...
	protected function select_option($table, $field_name) {
		...
		if (isset($table['where'])) {
			return '{:select_option("' . base64_encode(json_encode($table, true)) . '",$info["' . $field_name . '"])}';
		} else {
			$option = '';
			foreach ($table as $key => $value) {
				$option .= '<option value="' . $key . '" {if($info["' . $field_name . '"] eq "' . $key . '")}selected="selected"{/if} >' . $value . '</option>' . "\n";
			}
			return $option;
		}
	}
